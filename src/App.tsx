...
function App() {
  ...
  const PWA_URL = 'https://appv1-4fa.pages.dev/';
  ...
  useEffect(() => {
    ...
    // Event listener for Android PWA installation prompt
    ...
    <div className="app">
      ...
      <main id="main">
        ...
        {isMobile && !isStandalone && (
          <section className="pwa-section reveal visible" aria-labelledby="pwa-title">
            <div className="container">
              <div className="pwa-card">
                <div className="pwa-card-content">
                  <div className="pwa-icon-badge">📱</div>
                  <h2 id="pwa-title">
                    {isIOS ? 'Install on Your iPhone' : 'Install SchoolSync'}
                  </h2>
                  <p>
                    {isIOS 
                      ? 'No App Store needed — add it directly to your Home Screen in 3 steps' 
                      : 'Add SchoolSync directly to your Home Screen for the best experience'}
                  </p>
                  
                  <button 
                    onClick={handlePwaInstall} 
                    className={`btn-pwa-install ${isIOS ? 'ios' : 'android'}`}
                  >
                    {isIOS ? (
                      <>
                        <svg className="apple-logo-svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 4.17c.66-.81 1.11-1.93.99-3.06-.96.04-2.13.64-2.82 1.45-.6.69-1.12 1.84-.98 2.94.13.11.26.17.38.17.86 0 1.99-.61 2.43-1.5z"/>
                        </svg>
      ...
      {/* PWA SAFARI MODAL */}
      {showPwaModal && (
        <div className="pwa-modal-overlay" onClick={() => setShowPwaModal(false)} role="dialog" aria-modal="true">
          <div className="pwa-modal-card" onClick={(e) => e.stopPropagation()}>
            <button className="pwa-modal-close" onClick={() => setShowPwaModal(false)} aria-label="Close guide">
              <X size={20} />
            </button>
            ...
            <button className="btn-pwa-gotit" onClick={() => setShowPwaModal(false)}>
              Got it!
            </button>

            <div className="pwa-bounce-arrow" aria-hidden="true">
